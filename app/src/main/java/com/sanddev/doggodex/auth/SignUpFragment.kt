...
class SignUpFragment : Fragment() {
    ...
    private fun validateFields() {
        ...
        if (passwordConfirmation.isEmpty()) {
            binding.confirmPasswordInput.error = getString(R.string.password_must_not_empty)
            return
        }

        if (password != passwordConfirmation) {
            binding.confirmPasswordInput.error = getString(R.string.passwords_must_match)
            return
        }
